...
## 2.1. Kittiwakes, all sites, OD corrected for inter-plate variaions
...
## 2.2. Guillemots, all sites, OD corrected for inter-plate variaions
...
ggplot() + 
  geom_histogram(data = subset(data_corrected, !is.na(species)), aes(x = OD_adj, fill = plate),
                 binwidth=.1,size=0.25,
                 colour="black")+
    ylab("Number of samples")+xlab("Uncorrected optical density (anti-Bbsl ELISA)")+
  ggtheme+
  theme(legend.justification=c(1,1),legend.position=c(1,1)) +
  xlim(c(0,6)) +
...
ggplot() + 
  geom_histogram(data = subset(data_corrected, !is.na(species)), aes(x = OD_cor, fill = plate),
                 binwidth=.1,size=0.25,
                 colour="black")+
    ylab("Number of samples")+xlab("Corrected dptical density (anti-Bbsl ELISA)")+
  ggtheme+
  theme(legend.justification=c(1,1),legend.position=c(1,1)) +
  xlim(c(0,6)) +
  facet_grid(site ~ species)
...
## 3.1. Kittiwakes, Ossian only, OD corrected for inter-plate variaions
...
## 3.4. Guillemots, Ossian only, OD corrected for inter-plate variaions
...
## 3.2. Kittiwakes, all sites, uncorrected OD

```{r,warning=FALSE,message=FALSE,fig.width=14*0.4,fig.height=7*0.4}
data_rissa_complete_uncor = aggregate(OD_adj~sample_id + site, subset(data_corrected, species == "Rissa tridactyla"), FUN = "mean")
data_uria_complete_uncor = aggregate(OD_adj~sample_id + site, subset(data_corrected, species == "Uria lomvia"), FUN = "mean")

data_rissa_ossian_uncor = subset(data_rissa_complete_uncor, site == "Ossian")
data_uria_ossian_uncor = subset(data_uria_complete_uncor,  site == "Ossian")

data = data_rissa_complete_uncor
# fits two normal distributions to the corrected OD values
mixnorm<-normalmixEM(subset(data,!is.na(OD_adj))$OD_adj,eps=1e-12,lambda=0.5,mu=c(0,1),sigma=c(0.1,1),k=2)
# fits one normal distribution to the same data as above
norm1<-fitdistr(subset(data,!is.na(OD_adj))$OD_adj,"normal")
# manually calculate AIC 
AIC1n<-2*2-2*norm1$loglik
AIC2n<-4*2-2*mixnorm$loglik
c(AIC1n,AIC2n)
...
mixnorm_rissa_complete_uncor = mixnorm
cut95_rissa_complete_uncor=mixnorm_rissa_complete_uncor$mu[1]+2*mixnorm_rissa_complete_uncor$sigma[1]
cut99_rissa_complete_uncor=mixnorm_rissa_complete_uncor$mu[1]+3*mixnorm_rissa_complete_uncor$sigma[1]


# plot
x= seq(0, 6, 0.01)
pred_rissa_complete_uncor = data.frame(cbind(x = x, y1 = dnorm(x,mean=mixnorm_rissa_complete_uncor$mu[1],sd=mixnorm_rissa_complete_uncor$sigma[1])*2.5, y2 = dnorm(x,mean=mixnorm_rissa_complete_uncor$mu[2],sd=mixnorm_rissa_complete_uncor$sigma[2])*2.5))
ggplot() + 
  geom_histogram(data = data_rissa_complete_uncor, aes(x = OD_adj, fill = site),
                 binwidth=.1,size=0.25,
                 colour="black")+
  geom_line(data = pred_rissa_complete_uncor, aes(x=x, y=y1, colour="#8c34a9"), size = 0.75, alpha = 0.7) +
  geom_line(data = pred_rissa_complete_uncor, aes(x=x, y=y2, colour="#9ed99a"), size = 0.75, alpha = 0.7) +
  geom_vline(aes(xintercept=cut99_rissa_complete_uncor),linetype="dashed",size=0.5)+
  geom_vline(aes(xintercept=cut95_rissa_complete_uncor),linetype="dashed",size=0.5)+
  ylab("Number of samples")+xlab("Mean optical density (anti-Bbsl ELISA)")+
  ggtheme+
    scale_fill_manual(values = c("gray90", "grey50"), labels = c("Hornøya", "Ossian")) +
  scale_color_manual(values=c("#9ed99a","#8c34a9"),labels=c("ELISA-negative sample distribution","ELISA-positive sample distribution"))+
...
## 3.4. Guillemots, all sites, uncorrected OD

```{r,warning=FALSE,message=FALSE,fig.width=14*0.4,fig.height=7*0.4}
data = data_uria_complete_uncor 
# fits two normal distributions to the corrected OD values
mixnorm<-normalmixEM(subset(data,!is.na(OD_adj))$OD_adj,eps=1e-12,lambda=0.5,mu=c(0,1),sigma=c(0.1,1),k=2)
# fits one normal distribution to the same data as above
norm1<-fitdistr(subset(data,!is.na(OD_adj))$OD_adj,"normal")
...
mixnorm_uria_complete_uncor = mixnorm
cut95_uria_complete_uncor=mixnorm_uria_complete_uncor$mu[1]+2*mixnorm_uria_complete_uncor$sigma[1]
cut99_uria_complete_uncor=mixnorm_uria_complete_uncor$mu[1]+3*mixnorm_uria_complete_uncor$sigma[1]

pred_uria_complete_uncor = data.frame(cbind(x = x, y1 = dnorm(x,mean=mixnorm_uria_complete_uncor$mu[1],sd=mixnorm_uria_complete_uncor$sigma[1])*5, y2 = dnorm(x,mean=mixnorm_uria_complete_uncor$mu[2],sd=mixnorm_uria_complete_uncor$sigma[2])*5))
ggplot() + 
  geom_histogram(data = data_uria_complete_uncor, aes(x = OD_adj, fill = site),
                 binwidth=.1,size=0.25,
                 colour="black")+
  geom_line(data = pred_uria_complete_uncor, aes(x=x, y=y1, colour="#8c34a9"), size = 0.75, alpha = 0.7) +
  geom_line(data = pred_uria_complete_uncor, aes(x=x, y=y2, colour="#9ed99a"), size = 0.75, alpha = 0.7) +
  geom_vline(aes(xintercept=cut99_uria_complete_uncor),linetype="dashed",size=0.5)+
  geom_vline(aes(xintercept=cut95_uria_complete_uncor),linetype="dashed",size=0.5)+
  ylab("Number of samples")+xlab("Mean optical density (anti-Bbsl ELISA)")+
  ggtheme+
    scale_fill_manual(values = c("gray90", "grey50"), labels = c("Hornøya", "Ossian")) +
  scale_color_manual(values=c("#9ed99a","#8c34a9"),labels=c("ELISA-negative sample distribution","ELISA-positive sample distribution"))+
...
rmarkdown::render("C:/Users/Gamble/PhD Eco-epidemiology/On_going/data_and_analyses/deers/3_analyses/ELISA_borrelia_deers.rmd",output_format="html_document")